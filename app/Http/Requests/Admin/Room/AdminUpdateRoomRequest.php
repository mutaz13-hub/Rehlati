...
class AdminUpdateRoomRequest extends ApiFormRequest
{
    ...
    {
        return [
            'name_en' => ['required', 'string', 'max:255', Rule::unique('rooms', 'name_en')->ignore($this->route('room')->id)->where(function ($query) {
                return $query->where('hotel_id', $this->route('room')->hotel_id);
            })],
            'name_ar' => ['required', 'string', 'max:255', Rule::unique('rooms', 'name_ar')->ignore($this->route('room')->id)->where(function ($query) {
                return $query->where('hotel_id', $this->route('room')->hotel_id);
            })],
